...
void Back(int k, int *x, std::string cuv, double &entropie)
{
    ...
    if (k > 4)
    {
        /*
            ...
            -> 3. PROBABILITATE = (NR CUV NUMARATE / NR CUV TOTAL)
...
int Write(std::string str)
{
    ...
    if (fdw == -1)
    {
        perror("Eroare la deschidere fifo => Write");
        return -1;
    }
    // scriem in fifo
    char ch[5];
    ...
    {
        perror("Eroare la afisare in fifo!");
        return -1;
    }
...
int main()
{
    ...
    if (InitFiles() != 0)
    {
        perror("A aparut o eroare la fisiere!");
    ...
    while (ghicit == false)
    {
        ...
        if (nr_cuv == 1)
        { // Daca ne-a mai ramas un cuvant in lista atunci el este guess-ul nostru
            f1_in >> cuv;
            guess = cuv;
        }
